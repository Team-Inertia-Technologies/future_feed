...
try {
    ...
    if (sql_num_rows($video_query) === 0) {
        respond(404, ["statusCode" => 404, "error" => ["message" => "Comment not found."]]);
    }

    $video       = sql_fetch_assoc($video_query);
    $video_title = $video['vComment'];
    ...
    $already = sql_query("
        ...
        WHERE iUserID  = '$userid' 
        AND   iCommentID = '$CommentID' 
        AND   cStatus  = 'A' 
        LIMIT 1
    ");

    if (sql_num_rows($already) > 0) {
        respond(409, ["statusCode" => 409, "error" => ["message" => "You have already reported this comment."]]);
    }
    ...
    $report_id   = NextID('iReportID', 'comment_reports');

    sql_query("
        INSERT INTO comment_reports (iReportID, iUserID, iCommentID, vReason, dReportedAt, cStatus)
        VALUES ('$report_id', '$userid', '$CommentID', '$reason_esc', '$reported_at', 'A')
    ");
          ...
          <tr>
            <td style='padding:44px 44px 40px;'>
              ...
              <p style='margin:0 0 28px; font-size:13.5px; color:#9b8db0; line-height:1.6;'>
                A user has reported a comment on your platform. Below are the details of the report for your review.
              </p>
              ...
              <table cellpadding='0' cellspacing='0' border='0' width='100%'
                ...
                <tr>
                  <td style='padding:24px 24px 8px;'>
                    ...
                    <table width='100%' cellpadding='0' cellspacing='0' border='0' style='margin-bottom:14px;'>
                      <tr>
                        <td style='font-size:12px; color:#9b8db0; width:40%;'>Comment</td>
                        <td style='font-size:13px; color:#ffffff; font-weight:600;'>$video_title</td>
                      </tr>
                    </table>
...
";
    ...
    $subject     = "🚩 Comment Report #$report_id - $site_title";
    ...
    if (empty($result_data['messageId'])) {
        error_log("Comment report email failed for report #$report_id: " . json_encode($result_data));
    }
